...
show_help() {
    cat << EOF
Usage: glm-code.sh [OPTIONS]

Run Claude Code with GLM API in an isolated Docker environment.
...
Examples:
  glm-code.sh
  glm-code.sh --with ../backend
  glm-code.sh --with ../backend --with ../frontend

Inside the container:
  Current project: Read-write access at /workspace/<dirname>
...
eval docker run -it --rm --platform linux/amd64 \
  ...
  -v \"$CURRENT_PATH:/workspace/$CURRENT_DIR\" \
  --env-file \"$ENV_FILE\" \
